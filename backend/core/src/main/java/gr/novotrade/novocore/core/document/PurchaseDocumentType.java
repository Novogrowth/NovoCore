 ...
 * aggregate owns.
 *
 * <p>⚠️ {@link #affectsStock} is meaningful on this side and is not a mirror of the sales column:
 * {@code 2041 Δελτίο Παραλαβής} brings stock in with no payable behind it. Both flags are boxed on
 * purpose — a primitive would make "undecided" unrepresentable and turn every unanswered question
 * into a {@code false} that reads as a decision.
 */
@Entity
@Table(name = "purchase_document_type")
class PurchaseDocumentType extends AuditableEntity {
    ...
    /** For JPA only. */
    protected PurchaseDocumentType() {
    }
