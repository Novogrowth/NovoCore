...
- **Backend Phase 1: steps 0–5 done (plus inserted steps 3b and 4b). Step 6 is next, blocked on Q7 and Q25.**
  ...
  - **356 tests passing, `mvn clean verify` exit 0.** Compose stack verified healthy over HTTPS.
...
- **Product:** own SKU (unique) and EAN (unique when present), `GOODS`/`SERVICE`, unit of measure, a **required** default VAT class (there is no fallback rate anywhere, so a product without one could not be invoiced), an optional selling price, one optional supplier plus that supplier's own code. **No Go/Woo ids, no stock column, no last-purchase-price column** — both derived from lots (step 6). **A zero price is refused**; null means "not priced yet", because zero produces an invoice line worth nothing without anyone choosing to give the goods away.
...
- **Q7 — how stock is exposed** (per location plus a "sellable" figure?) and **Q25 — write-off reason as enum or free text**. These two now block step 6, and nothing about stock was built in step 5 because of Q7.
- **Needs the accountant: statutory depreciation rates per asset category**, plus the category taxonomy they attach to. The per-asset rate field exists and is nullable — **do not create real assets with real values until these are confirmed**, the same caution the VAT classes needed.
- **Q33 (new)** — should a delivery or COD fee follow the VAT rate of the goods it delivers? Greek practice says an ancillary charge does; the seeded 24% default is wrong on a 13% order and nothing derives the right rate. Interacts with Q14.
- **Q34 (new)** — should `UnitOfMeasure` be a runtime-editable lookup rather than an enum? Go holds it as an editable list and myDATA has its own unit codes. Cheap now, less so after step 6.
- **Q35 / Q36 (new)** — AADE exemption codes 24 and 28 are absent from Go's list, and the OSS/IOSS reasons have no myDATA code. Both need confirming before phase 7.
- **Q37 (new)** — Customer and Supplier have no address fields (not needed while Go issues invoices, needed by phase 11), no human-facing codes, and only one selling price per product.
