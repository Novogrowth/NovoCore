...
- **Backend Phase 1: steps 0–6 done (plus inserted steps 3b and 4b). Step 7 (the journal engine) is next, blocked on Q13 and Q14 — both of which need a design conversation rather than a one-line answer.**
  ...
  - **458 tests passing, `mvn clean verify` exit 0.** Compose stack verified healthy over HTTPS.
...
- **Not built, deliberately:** no source-document reference on a lot (the Goods Receipt is step 8, ADR 0004), no FIFO consumption, no write-off table, no valuation column, and Q17 (may aggregate stock go negative) is undecided — a single lot cannot, by CHECK.
...
- **Q13 (correction policy) and Q14 (VAT posting mechanics) now block step 7**, and both need a real conversation rather than a one-line answer. See below.
