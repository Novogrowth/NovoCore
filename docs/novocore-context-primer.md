...
- ~~**ChargeType income account (Q27)**~~ — **RESOLVED: dedicated `Delivery income` and `COD fee income` accounts, no channel split.** ⚠️ **Decision made, code not written** — neither account exists yet and `charge_type` is still empty. Outstanding: a `V7` migration adding both accounts (65 → 67) and seeding the two ChargeType rows. Needs no further input; good first task next session.
- **VatExemptionReason seed** — ~29 verified AADE rows. **Being supplied at the start of the next session.** Structure ready and waiting. Together with Q5, this is what blocks step 5.
...
- **Product↔Supplier link (Q5)** — Product has "Supplier's SKU" but no supplier reference, which makes the field meaningless. **A hard blocker for step 5's Product entity.** Note it blocks *Product* specifically: Customer still needs Q8/Q9 and Asset needs Q12, so answering Q5 alone unblocks roughly a quarter of step 5.
