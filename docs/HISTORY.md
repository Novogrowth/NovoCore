# NovoCore — Build History

> ## ⚠️ THIS FILE IS NOT AUTHORITATIVE FOR CURRENT STATE
>
> **It is append-only, and every entry records what one step did on the day it closed.**
> A figure, a status, a "next step" or a "still open" inside an entry was **true when it was written**
> and is framed by the entry it sits in — never lift one out and read it as current.
>
> **Current state lives in exactly one place: [`docs/PROGRESS.md`](PROGRESS.md).** Read that.
> The sequence and each step's scheduling glyph live in [`docs/novocore-roadmap.md`](novocore-roadmap.md).

*Split out of `PROGRESS.md` by **U2a, 2026-08-06**, when that file was 9,577 lines and the first thing
every session read. **Nothing here was rewritten, condensed or summarised** — the live sections were
relocated whole, each leaving a dated pointer where it stood. Everything else is byte-identical.*

**Appending a new step:** the entry goes **at the top**, directly under the index, and gains an index
row. Corrections to a closed entry are **appended inside it, dated, never edited in place** — which is
what every `⚠️ CORRECTED` line already in this file is.

---

## Index — by step identifier

**The step id is the key; the date is a column.** This index is a navigation aid written by U2a. It
carries **no status column, deliberately** — a second table with a status is how this project's records
came to disagree. For status, read `PROGRESS.md`.
...
| **W1 part 1** | 2026-08-04 | The roadmap re-sequenced, and the rule that a sequencing decision changes row ORDER |
| **R2** | 2026-08-04 | Six settings screens over R1a's tables; grew block X — editable while unused, frozen once used |
| **R1b** | 2026-08-04 | `seriesId` mandatory, channel derived from the series, the silent `affectsStock` branch (**V33**) |
| **U3** | 2026-08-03 | Eleven design decisions written into the repository. Documentation only |
| **8a** | 2026-08-03 | `@Mandatory` — 339 components across 114 records, bytecode cross-check, 75 → 143 schemas declaring `required` |
| **R1 Phase 0** | 2026-08-03 | Eleven corrected premises, measured against the running stack and the AADE artefacts |
| **R1a** | 2026-08-03 | Document reference data, additive — six tables, 54 routes (**V31**, **V32**) |
| **R1** | 2026-08-03 | R1's approved scope and its six decisions |
| **R1 model** | 2026-08-03 | *Why the model changed* — the two-layer correction, and R1's rewritten Part 3 checklist |
| **Q1** | 2026-08-03 | The backend follow-up queue — items 4+6, 5, 1, 7 |
| **U1** | 2026-08-02 | Roadmap unification, and fourteen decisions that existed only in chat |
| **U1b** | 2026-08-02 | U1's six follow-up corrections |
| **F4** | 2026-08-01 | Settings — three config pages, VAT classes, units of measure (**V30**) |
| **S2** | 2026-08-01 | Sortable columns on five screens; the collation question settled from the live database |
| **S1** | 2026-08-01 | Substring search — `pg_trgm` + `unaccent` (**V28**, **V29**) |
| **CI** | 2026-07-29 | `pg_dump` made to actually mean 17 on the runner |
| **16a** | 2026-07-30 | Backend prerequisites — `/api/me`, the preview endpoints, the OpenAPI spec, the paging contract (**V27**) |
| **16b** | 2026-07-30 | Users & roles, journal listing, settings — 37 routes |
| **Sessions** | 2026-07-30 | A permission change now takes effect immediately — session eviction |
| **Commits** | — | **The commit → step index.** Which commit each step landed in |
| | | ⚠️ **THE SEAM IS HERE — the direction reverses below. See the note after this table** |
| **3** | — | Chart of accounts — 65 accounts across 13 groups (**V4**) |
| **3b** | — | VAT classes, exemption reasons, charge types (**V5**) |
| **4** | — | Users, auth, permissions (**V6**) |
| **4b** | — | The first REST endpoint, and the web boundary rule made real |
| **V7 / V8** | — | Delivery and COD income accounts; the real AADE exemption-reason seed |
| **5** | — | Product, Customer, Supplier, Asset (**V9**) |
| **V10 / V11** | — | The VAT rate bound's blind spot; units of measure became a table |
| **6** | — | Inventory lots, serialized units, locations, bundles (**V12**, **V13**) |
| **7** | — | The journal engine; debits = credits as a deferred constraint trigger (**V14**, **V15**) |
| **8** | — | Purchase invoices, goods receipts, GR/IR, variance, FIFO (**V16**) |
| **9** | — | Sales invoices, credit notes, settlements, open items, rounding (**V17**) |
| **10** | — | Freight / landed cost allocation (**V18**, **V19**) |
| **11** | — | The shared email service — outbox, dispatcher, retry (**V20**) |
| **11b** | — | An emailed document is referenced, not copied (**V21**) |
| **Q43** | — | Retention — rows forever, generated attachments 90 days (**V22**) |
| **12** | 2026-07-29 | Automated backups — encrypted, off-site, proven restorable (**V23**) |
| **Proxy** | — | Proxy self-invocation made a build failure, and the two real defects it found |
| **13** | — | The property harness, FIFO properties, the whole-scenario invariant sweep |
| **13b** | — | Q45 — a lot self-liquidates to exactly zero (**V24**) |
| **Questions** | — | **The RESOLVED questions, filed at the step that answered each.** The still-open ones are in `PROGRESS.md` |
| **14** | — | The REST surface — 133 routes, and Q44 in full (**V25**) |
| **15** | 2026-07-30 | Dummy data validation over HTTP — nine real defects (**V26**) |
| **16** | 2026-07-31 | The frontend — foundations, Products, the brand pass |
| **F0** | 2026-07-31 | The dev database, and the approved commit nobody missed — 15c had never been written |
| **Wedge** | 2026-07-31 | The Products bugfix pass — the `DataTable` render loop |
| **Queue** | 2026-07-30 → 2026-08-03 | ⚠️ **The backend follow-up queue, as raised and as closed.** Its heading says *"read this first"* and means neither — see the dated correction above it |

### ⚠️ The seam — this file runs in two directions, and that is a fact about how it grew

**From the top down to `Git state`, entries are NEWEST FIRST.** From `Step 3` onward they are **OLDEST
FIRST**, because that is the order `PROGRESS.md` accumulated them in before U2a.

**U2a deliberately did not reorder either half.** Normalising the direction would have reversed either
3,396 or 4,766 lines for a benefit the index above already delivers, and would have turned a reviewable
relocation into a diff nobody could check. **The index is the addressing mechanism; physical order is
...
*(**U2a, 2026-08-06.** `▶ R2c` moved to `docs/PROGRESS.md` — it is live, not history. Nothing was edited: the whole section was relocated.)*

*(**U2a, 2026-08-06.** `▶ R4 — the current step` moved to `docs/PROGRESS.md` — it is live, not history. Nothing was edited: the whole section was relocated.)*
...
*(**U2a, 2026-08-06.** `▶▶ What is next, in one place` moved to `docs/PROGRESS.md` — it is live, not history. Nothing was edited: the whole section was relocated.)*

*(**U2a, 2026-08-06.** That table and `Where things stand` were **merged into the single status table** in `docs/PROGRESS.md`, per U2a's C.2. Neither survives as a separate table.)*

*(**U2a, 2026-08-06.** `⚖️ Held for the owner — test-environment parity` moved to `docs/PROGRESS.md` — it is live, not history. Nothing was edited: the whole section was relocated.)*
...
*(**U2a, 2026-08-06.** `📌 STANDING OPEN OBLIGATION — the database does not yet sort the way the browser does` moved to `docs/PROGRESS.md` — it is a specification whoever adds paging adopts. Nothing was edited: the whole section was relocated.)*
---
...
*(**U2a, 2026-08-06.** `🎯 The search target list` moved to `docs/PROGRESS.md` — it is the authoritative list every search-adding step adopts. Nothing was edited: the whole section was relocated.)*

*(The `CI` section below sat inside it and is history; U2a promoted its heading from `###` to `##` so it is not orphaned. Its title text is byte-identical.)*
...
*(**U2a, 2026-08-06.** `🚫 Pre-launch blockers` moved to `docs/PROGRESS.md` — it is live — PLB-1 has a named trigger. Nothing was edited: the whole section was relocated.)*
...
*(**U2a, 2026-08-06.** `⚠️ To be aware of immediately` moved to `docs/PROGRESS.md` — it is live — operational traps a session acts on. Nothing was edited: the whole section was relocated.)*
## Git state
...
*(**U2a, 2026-08-06.** `Verified working` and `Not yet verified` moved to `docs/PROGRESS.md` — it is live, and they stay together because their meaning is the contrast between them. Nothing was edited: the whole section was relocated.)*
---
...
*(**U2a, 2026-08-06.** `⚠️ Still waiting on input` and `Q28 in full` moved to `docs/PROGRESS.md` — it is still open. Nothing was edited: the whole section was relocated.)*
### Resolved in step 4
...
---
*(**⚠️ U2a, 2026-08-06 — a dated correction, appended rather than edited.** The heading below reads
*"Next action — read this first"*. **It is neither.** It sat at line 8305 of a 9,577-line file — 87% of
the way down — and every one of its nine numbered items is closed. Its subheading further down still
says *"Q1 is next, then R1, then F5"*, which was stale by two steps when that was noted on 2026-08-02
and is stale by nine now. **The heading is left byte-identical deliberately** — this file is
append-only and a heading is an entry — and this correction is the record that it does not mean what
it says. Current status is in `docs/PROGRESS.md`.)*
...
*(**U2a, 2026-08-06.** `Waiting on the accountant` and `Also still open` moved to `docs/PROGRESS.md` — it is still open. Nothing was edited: the whole section was relocated.)*
