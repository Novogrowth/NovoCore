...
*Live status. **Overwritten each session close-out, never appended to.** Last updated: 2026-08-06 —
U2a split this file, and R4 is current.*
...
| U2b | **The split's drift guards** | 🔴 **NEXT after R4's close-out, and not optional.** Three tests — a step id has a `##` section in exactly one file; every `PROGRESS.md`/`HISTORY.md` reference names a file that exists; `HISTORY.md`'s *not authoritative* header is present — plus a test that **exactly one** table matches `\| Step \| … \| Status \|`. ⚠️ **And the CI path change: without it the guards never run on the change they guard**, which makes the other three decoration |
...
| F5b | **`el-GR-x-icu` on `DOCUMENT_NUMBER`** | ⚪ **DEFERRED out of F5 (its B.4), given a row by U2a 2026-08-06.** ⚠️ **It had no row before, and the primer wrongly called it one.** Conditional on whether a real Prosvasis Go document number carries Greek letters — **the owner's check, still outstanding**. The note naming where the `Pageable` path would have to be left is at `SalesInvoiceServiceImpl.SORTABLE` |
...
|---|---|---|
| **F5's B.4** | **F5b** | `ORDER BY … COLLATE "el-GR-x-icu"` on `DOCUMENT_NUMBER`. ⚠️ **Conditional on the owner's check** of whether a real Prosvasis Go document number carries Greek letters — **still outstanding**. ⚠️ **The primer called this a roadmap row when it was not** — a second record describing something that did not exist |
